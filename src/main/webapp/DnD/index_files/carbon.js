var _carbonads = {
	init : function(where, force_serve) {
		...
		if (where)
			_carbon_where = where;
		var placement = this.getUrlVar('placement'), serve = this.getServe(
				force_serve ? force_serve : this.getUrlVar('serve'), placement), baseurl = this
				.getUrlVar('cd') ? this.getUrlVar('cd') : 'srv.carbonads.net';
		var forcebanner = this.getUrlVar('bsaproforce', window.location.href), ignore = this
				.getUrlVar('bsaprostats', window.location.href), forwardedip = this
				.getUrlVar('bsaforwardedip', window.location.href);
		...
		if (day.length || life.length) {
			...
			for (var i = 0; i < freqcap.length; i++)
				freqcap[i] = freqcap[i][0] + ':' + freqcap[i][1] + ','
						+ freqcap[i][2];
			if (freqcap.length)
	...
	getServe : function(serve, placement) {
		var skip = [ 'andrewdumontme', 'bootstrapvalidatorcom',
				'cooldesignjobscom', 'craftedinorg', 'dbtek', 'hackerbracket',
				'launchbyte', 'markitupjaysalvatcom', 'nomadlistcom',
				'photoshopsecretstumblrcom', 'theinstructionalcom',
				'tympanusnet', 'wwwbuildinternetcom', 'wwwquenesscom',
				'wwwravelrumbacom', 'wwwsqlreplcom', 'codylindley',
				'yeungithubioopencolor', 'whatanartcom',
				'photoshopsecretstumblrcom', 'mapkitio', 'hackersomecom',
				'phalconphpcom' ];
	...
	getUrlVar : function(name, target) {
		target = typeof target !== 'undefined' ? target : document
				.getElementById('_carbonads_js').src, name = name.replace(
				/[\[]/, "\\\[").replace(/[\]]/, "\\\]");
		var regexS = "[\\?&]" + name + "=([^&#]*)";
...
function _carbonads_go(b) {
	var ad = b['ads'][0], link, fulllink, image, description, time = Math
			.round(Date.now() / 10000) | 0;
	var placement = _carbonads.getUrlVar('placement');
	var serve = _carbonads.getUrlVar('serve');
	if (ad.html != null) {
		var ad_html = JSON.parse(ad.html);
		ad.image = ad_html.image, ad.statlink = ad_html.statlink,
				ad.description = ad_html.description, ad.pixel = ad_html.pixel;
	...
	if (ad.fetch != null) {
		...
		if (ad.click_redir != null)
			fetch.src += '?click_redir='
					+ encodeURIComponent(ad.click_redir.replace(
							/srv.buysellads.com/g, 'srv.carbonads.net'));
	...
	if (!_carbonads.isset(ad.removecarbon))
		el.innerHTML += '<a href="http://carbonads.net/?utm_source='
				+ placement
				+ '&utm_medium=ad_via_link&utm_campaign=in_unit&utm_term=carbon" class="carbon-poweredby" target="_blank">ads via Carbon</a>';
